...
安装后直接使用斜杠命令：`/luban-school:wendao` `/luban-school:sinan` `/luban-school:luban` `/luban-school:digong` `/luban-school:mozi` `/luban-school:chiling-zhoutian` `/luban-school:chiling-xuntian`。中英映射详见下方"使用方法"。无需手动配置 CLAUDE.md。
